...
public class CommandRegister {
	...
	private List<String> getSkip() {
		...
		String file;
		try {
			file = Data.readFile(CDPlugin.getDir() + "/overridden",
	...
	public void searchCommands(boolean registerToBukkit) {
		clog.log("Searching for Commands", this);
		...
		for (CDPlugin cdp : cdpp.handler.plugins.values())
			for (Method m : cdp.getClass().getMethods()) {
				if ((pc = m.getAnnotation(CDPluginCommand.class)) == null)
					continue;
				for (String command : pc.commands()) {
					if (!checkCommand(command)) {
						clog.log(
								"Commands have to have the Syntax 'COMMAND PERMISSION SHOWMSG(0 | 1). '"
										+ command
										+ "' does not have this Syntax and would not be registered.",
								this);
						continue;
					}
					if ((list = clistener.commands.get(command.toLowerCase())) == null)
						list = new ArrayList<Entry<Object, Method>>();
					list.add(new MyEntry<Object, Method>(cdp, m));
					clistener.commands.put(command.toLowerCase(), list);
					if ((aliases = clistener.commandList
							.get((simpleCmd = parseToCommand(command)))) == null)
						aliases = new ArrayList<String>();
					aliases.add(command);
					clistener.commandList.put(simpleCmd, aliases);
					clog.log("Registering '" + command + "' Command to "
							+ cdp.getClass().getSimpleName(), this);
				}
			}
		sortCalls();
		clog.log("Done", this);
	...
	public void registerBukkitCommands() {
		clog.log("Register Commands to Bukkit", this);
		Method m;
		try { m = clistener.getClass().getMethod("onCommand", CommandEvent.class); }
		catch (NoSuchMethodException x)
		{
			clog.log(
					"Method CommandListener.onCommand(CommandEvent) not found. Commands are not registered",
					this);
			return;
		}
	...
	private void registerBukkitCommand(String command, Method m) {
		...
		if (skip.contains(command)) {
			clog.log("Skipping Command " + command
					+ " because he is marked as overridden", this);
			return;
		}
	...
	private SimpleCommandMap getCommandMap() {
		...
		try {
			return (SimpleCommandMap) Bukkit.getServer().getClass()
